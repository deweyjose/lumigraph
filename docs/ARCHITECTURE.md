...
- **Astro Hub chat observability (#167):** Each request gets a `chatRunId` (UUID) on the server; it is passed as optional `toolContext.chatRunId` and logged at run start (`event: chat_run_start`) with `userId` via `[chat]`-prefixed structured logs in `apps/web/src/server/chat-log.ts`. Each Responses round logs `responses_round` (OpenAI `responseId`, chained `previousResponseId`, function tool names, `webSearchCallCount` from output item types only—no queries, URLs, or bodies). Failed `astro_hub_*` tools log `event: tool` at **warn** with `code` and `durationMs`. Successful tool executions are not logged unless `LUMIGRAPH_CHAT_DEBUG` is `1` or `true` (adds per-tool success lines and stack traces for thrown errors in the chat path). **Do not** log user/assistant message text, stream deltas, or full tool payloads. A terminal `chat_run_complete` line summarizes round count, deduped tool names used, and citation count.
